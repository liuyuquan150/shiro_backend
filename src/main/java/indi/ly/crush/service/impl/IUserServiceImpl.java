 ...
 *     这意味着即使是一个空的字符串实例也可能在以后的时间(例如{@code 内存转储——memory dump})中以内存的形式被访问, 这对密码等敏感信息来说是不利的. <br /> <br />
 *
 ...
 *              因为 {@link String} 对象不可变, 所以即使是你不再需要密码信息(比如{@code 用户认证之后}), 密码原始内容仍然存储在内存中, 直到 {@code JVM} 进行垃圾回收. <br />
 *              在这段时间里, 如果有人能够访问到内存转储, 就可能读取到这些敏感信息.
 *          </li>
 *          <li>
 *              使用 {@code char[]} 的优势. <br /> <br />
 *
 *              相比之下, 字符数组 {@code char[]} 是可变的. <br />
 *              这意味着你可以在使用完密码信息之后, 通过遍历数组并显式地将每个元素设置为零或其它值来{@code 清除}这些信息, 从而减少敏感数据被意外泄露的风险.
 *          </li>
 *      </ol>
